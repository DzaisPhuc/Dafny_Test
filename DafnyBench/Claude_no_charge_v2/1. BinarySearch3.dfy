  ...
  ensures count <= arr.Length
{
  ...
  // Prove that lower <= upper
  // LowerBound: forall i :: 0 <= i < lower ==> arr[i] < target
  // UpperBound: forall i :: 0 <= i < upper ==> arr[i] <= target
  // Therefore: if lower > upper, there exists i where upper <= i < lower
  // For such i: arr[i] <= target (from UpperBound) and arr[i] < target (from LowerBound)
  // This is impossible, so lower <= upper
  if lower > upper {
    assert forall i :: 0 <= i < lower ==> arr[i] < target;
    assert forall i :: 0 <= i < upper ==> arr[i] <= target;
    assert exists i :: upper <= i < lower;
    assert false;
  }
